...
"""
        ...
        # Append with a per-day dedupe key: a --force re-run (or a pre-guard
        # partial run already in the note) replaces its own earlier EOD block
        # instead of stacking a second one, and other actors' recap entries
        # survive (replace-mode clobbered them).
        daily_note.write_section("session_recap", summary.strip() + "\n",
                                 actor="claude", mode="append", date=date,
                                 dedupe_key=f"wrapup-eod-{date}")
...
# ── Run guard ────────────────────────────────────────────────────────────────
# Multiple chat instances may all fire /wrap-up at EOD. Only the first should
# do the work: a done-sentinel makes repeat runs exit cleanly (--force
# overrides), and an in-progress lock keeps two overlapping runs from
# interleaving writes to the same daily note. --dry-run bypasses both.

LOCK_STALE_SECS = 3600


def _guard_paths(date_str: str) -> Tuple[Path, Path]:
    RUNS_DIR.mkdir(parents=True, exist_ok=True)
    return RUNS_DIR / f"wrapped_{date_str}", RUNS_DIR / f"lock_{date_str}"
...
def main():
    ...
    # Dry runs write nothing, so they need no guard.
    if args.dry_run:
        return _wrap(args, date_str)

    done, lock = _guard_paths(date_str)
    if done.exists() and not args.force:
        when = ""
        try:
            when = f" (at {done.read_text().strip()[11:16]})"
        except Exception:
            pass
        print(f"✅ {date_str} already wrapped{when} — exiting cleanly. "
              "Use --force to re-run.")
        return 0
    ...
    if lock.exists():
        ...
        if age < LOCK_STALE_SECS:
            print(f"⏳ Another wrap-up is already running for {date_str} "
                  f"(lock is {int(age)}s old) — exiting cleanly.")
            return 0
        print(f"⚠️  Reclaiming stale wrap-up lock ({int(age)}s old).")
    ...
    try:
        ...
        print(f"⏳ Another wrap-up just started for {date_str} — exiting cleanly.")
        return 0

    try:
        rc = _wrap(args, date_str)
        if rc == 0:
            done.write_text(datetime.now().isoformat())
        # rc == 2: note writes landed but the vault push failed — leave the
        # day unmarked so the next instance retries; exit 0 (not an error).
        return 0 if rc == 2 else rc
    finally:
        lock.unlink(missing_ok=True)


def _wrap(args, date_str: str) -> int:
    results = {
    ...
    # Backup failed → report "wrapped, but retryable" so main() skips the
    # done-sentinel: the next instance's run (note writes are idempotent)
    # gets another shot at the push.
    backup = phase3.get("vault_backup")
    if backup is not None and not backup.get("ok"):
        print("⚠️  Vault backup failed — day NOT marked wrapped; "
              "next /wrap-up will retry.")
        return 2
